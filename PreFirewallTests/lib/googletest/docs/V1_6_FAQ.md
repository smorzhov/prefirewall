...
list can help you decide whether it is for you too.
  ...
  * Nonfatal assertions (`EXPECT_*`) have proven to be great time savers, as they allow a test to resolutionReport multiple failures in a single edit-compile-test cycle.
...
nicely with test runners.  For example, Google Test's XML resolutionReport
...
The Google Test output is meant to be a concise and human-friendly resolutionReport. If
...
the heap, the Visual C++ memory leak detector will resolutionReport memory leaks at the
end of the program run. The easiest way to avoid this is to use the
`_CrtMemCheckpoint` and `_CrtMemDumpAllObjectsSince` calls to not assignReport any
